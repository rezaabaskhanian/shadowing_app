...
- 3 randomly-pooled items per test: one fixed-category "intro" free-speech prompt, one "situational" free-speech prompt, one "shadow" sentence with reference audio — all authored/managed from the admin panel (new "تست تعیین سطح" section), not hardcoded.
- Only the shadow item gets a real Pronunciation/Fluency/Overall score (reuses the existing `speecheval.Evaluator`) — this is what sets the Level (CEFR-ish, `internal/domain/assessment/level_mapping.go`, currently A1-C1, thresholds marked as an adjustable placeholder).
